...
export class UpdateManager {
    private static checkUpdate: boolean = true;

    public setCheckUpdate(state: boolean) {
        UpdateManager.checkUpdate = state;
    }

    // 获取调试器lua文件的版本号，并提示用户升级
    public checkIfLuaPandaNeedUpdate(LuaPandaPath: string, rootFolder: string) {
            ...
            if (DVerArr.length === AVerArr.length && DVerArr.length === 3) {
                ...
                if (intDVer < intAVer && updateTipSetting !== false) {
                    ...
                    vscode.window.showInformationMessage(
                        'LuaPanda VSCode 插件已升级 3.2.0 版本, 建议同时升级 LuaPanda.lua 文件。首次开始调试前请重建一下 launch.json 文件, 避免产生兼容问题。launch.json 配置项目参考 https://github.com/Tencent/LuaPanda/blob/master/Docs/Manual/launch-json-introduction.md',
                        '好的'
                    );

                    vscode.window
                        .showInformationMessage('当前工程中的 LuaPanda.lua 文件版本较低，是否自动替换为最新版本?', 'Yes', 'No', 'Never')
                        .then(value => {
                            if (value === 'Yes') {
                                let confirmButton = '立刻升级';
                                vscode.window
                                    .showInformationMessage(
                                        '已准备好更新 ' + LuaPandaPath + '。如用户对此文件有修改, 建议备份后再升级, 避免修改内容被覆盖',
                                        confirmButton,
                                        '稍后再试'
                                    )
                                    .then(value => {
                                ...
                                // 永久不再提示升级
                                vscode.window.showInformationMessage(
                                    '本项目调试时将不会再弹出调试器升级提示，需要升级请参考 https://github.com/Tencent/LuaPanda/blob/master/Docs/Manual/update.md',
                                    '好的'
                                );
                                this.setCheckUpdate(false);
                                // 把信息标记在 launch.json上
    ...
    public updateLuaPandaFile(LuaPandaPath: string) {
        //文件替换
        let luapandaContent = fs.readFileSync(Tools.getLuaPathInExtension());
        try {
            fs.writeFileSync(LuaPandaPath, luapandaContent);
            Ui.logging('升级成功, ' + LuaPandaPath + ' 已升级到 ' + Tools.adapterVersion);
        } catch (error) {
            Ui.logging('升级失败, ' + LuaPandaPath + '写入失败! 可以手动替换此文件到github最新版');
        } finally {
            this.setCheckUpdate(false);
        }
    }
