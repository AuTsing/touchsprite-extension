...
export interface ICoordinateContext {
    ...
    c: string;
    preview: string;
    updateCoordinate: (x: number, y: number, activeJimp: Jimp) => void;
}

export const CoordinateContext = createContext<ICoordinateContext>(undefined);
...
const CoordinateContextProvider = (props: { children: React.ReactNode }) => {
    ...
    const updateCoordinate = (x0: number, y0: number, activeJimp: Jimp) => {
        if (!activeJimp) {
            return;
        }
    ...
    return <CoordinateContext.Provider value={{ x, y, c, preview, updateCoordinate }}>{props.children}</CoordinateContext.Provider>;
};
